								...
								<c:otherwise>
									...
									<c:choose>
										<c:when test="${not empty sentOTP }">
											...
											<!-- ===form for user email verification pasword == -->

											<form action="verify_user_otp" method="POST">
											...
											<a href="${pageContext.servletContext.contextPath}/resenduserdotp" style="left:85%;position: sticky;">resend OTP</a>
										</c:when>
										...
										<c:choose>
											<c:when test="${not empty otpInvalid }">
												<div class="alert alert-success" role="alert">
													<p class="text-center" style="color: red">${otpInvalid }</p>
												</div>
												<!-- ===form for user email verification == -->
